...
class OmniTableController {
  constructor($scope) {
    'ngInject'
    if (!this.columns) {
      throw new Error('OmniTableController: columns must be provided');
    }
    // search是空数组时重置为null
    ...
    this.formatRowSelection(this.rowSelection);
    this.render(this.datasource);

    $scope.$watch(() => this.datasource, (newValue) => {
  ...
  render(datasource) {
    ...
    this.calculateValidDataLength(datasource);
    this.formatData(datasource);
  }
  ...
  formatData(data) {
    ...
    let isDefaultSelectedRowKeysEmpty = false;

    //如果没有默认选中的，需要format，disabled
    if (!this.defaultSelectedRowKeys || (this.defaultSelectedRowKeys && !this.defaultSelectedRowKeys.length)) {
      isDefaultSelectedRowKeysEmpty = true;
    }
    ...
    data.forEach(item => {
      ...
      if (!isDefaultSelectedRowKeysEmpty) {
  ...
  /**
   * 为没有 onSelect 或 getCheckboxProps的rowSelection添加一个默认的getCheckboxProps onSelect
   * @param columns
   */
  formatRowSelection(rowSelection) {
    if (!rowSelection) {
      return
    }
    ...
    if (!this.rowSelection.onSelect || typeof this.rowSelection.onSelect != 'function') {
      this.rowSelection.onSelect = () => {
      }
    }

    if (!this.rowSelection.getCheckboxProps || typeof this.rowSelection.getCheckboxProps != 'function') {

...
describe('Select', () => {
  ...
  describe('测试ngModel', () => {
    ...
    it('组件初始化后，input会显示ngModel的相应内容', () => {
  ...
  describe('测试组件', () => {
    ...
    describe('测试远程异步获取数据的方式,', () => {
      ...
      it('点击的数据是从remoteMethod获取', inject(($timeout) => {
        finder.find('.f-input').triggerHandler('click');
        $timeout.flush();
        let items = finder.items();
      ...
      it('输入值，并触发onChange, 结果值回根据输入值filter', inject(($timeout) => {
       ...
       */
      it('输入无结果并且失焦时，input应该显示的是ngModel的值', () => {
        finder.find('.f-input').triggerHandler('click');
       ...
       */
      it('点击获取数据时，直到remoteMethod数据返回时，才打开dropdown', inject(($timeout) => {
        finder.find('.f-input').triggerHandler('click');
      ...
      it('每次change值的时候，都重新打开drowDown',inject(($timeout)=>{
        console.log(finder.find('.f-select').hasClass('active'))
        ...
        finder.find('.f-input').triggerHandler('click');
        console.log(finder.find('.f-select').hasClass('active'))
        expect(finder.dropDown()).not.to.be.emty
      }))
    })
    ...
    describe('测试 is-multiple', () => {
      ...
      it('当点击标签区域（f-select_tags）时，聚焦输入框，并且打开菜单', () => {
    ...
    describe('测试 allow-create', () => {
      let finder;
      beforeEach(() => {
        $rootScope.data = new MockData().getData(false);
        $rootScope.city = {};
        element = $compile('<f-select data="data" ng-model="city" allow-create="true" is-multiple="true" ></f-select>')($rootScope);
        finder = new ElementFinder(element);
        $rootScope.$digest();
      })

      it('输入不存在的值，列表长度应为1，并且可添加', inject(($timeout) => {
  ...
  describe('测试directive', () => {
    ...
    describe('测试calculate-style', () => {
      ...
      it('当选中多个item时，高度在增加', inject($timeout => {
        finder.find('.f-input').triggerHandler('click');

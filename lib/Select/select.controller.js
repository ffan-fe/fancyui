...
class SelectController {
  ...
  onClick(e) {
    ...
    if (!this.active) {
      /**
       * 因为remote方法需要通过change去远程获取数据；
       * 所以点击click时，对于remote来说，需要额外执行onInputChange.
       * 并且远程的方法里，自己去绑定dismissClickhandler，保证了打开dropdown时，肯定是可以被关闭的
       */
      if (this.isRemote) {
        this.onInputChange(dismissClickHandler);
      }
      else{
  ...
  onInputChange(dismissClickHandler) {
    ...
    this.remoteMethod({
      ...
      /**
       * 当使用静态搜索的时候，需要copy，不然污染数据源
       */
      this.dataArray = this.buildArray(angular.copy(data));
      this.isOpen = true;
